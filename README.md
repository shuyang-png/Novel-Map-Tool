...
随着小说创作推进，地图会经历多次重大变化（城池被毁、势力扩张、战争遗迹等），**剧情回溯功能**允许你随时回溯到任意章节的地图状态。

- **创建版本快照**：每次剧情变化后，为当前地图状态创建快照，支持填写章节号（如"第3章"）和简短标签（如"主角破城"）
- **自动计算变更**：创建版本时自动对比上一版本，记录新增了哪些坐标/地理标识、修改了哪些内容、删除了什么。无变更则不创建版本
- **版本时间轴**：左侧面板以时间线形式展示所有版本，点击任意版本即可回溯查看该时刻的完整地图状态
- **安全查看模式**：查看历史版本时禁止任何编辑操作，可随时退出回到最新版本继续创作
- **版本对比**：支持对比任意两个版本的差异，直观看到哪些标记新增、删除、修改过
